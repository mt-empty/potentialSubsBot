/**
 ...
 // configuration options: request delay, so the script doesn't get rate limited by reddit API
  ...
  * at each iteration, it checks whether it has found the required number of matches for a post submission
  */
 async function main() {
     ...
     }, constants.API_RATE_INTERVAL)
 
 
     // post at least every POST_INTERVAL hours
     let postInterval = setInterval(() => {
         if (weeklyPotentialSubreddits.getSize() >= constants.CREATE_POST_THRESH || (removeCREATE_POST_THRETH && weeklyPotentialSubreddits.getSize())) {
             submitBotSelfpost(weeklyPotentialSubreddits.getPotentialArray(constants.CREATE_POST_THRESH))
         }
     }, constants.POST_INTERVAL)
 ...
 function SearchForSubreddit(subredditName, comment) {
     rClient.searchSubredditNames({ query: subredditName, exact: true }) // throws an error if the subreddit doesn't exist
         ...
         .catch((error) => {  // catching error means the subreddit doesn't exist
             if (error.statusCode == 404) {
                 ...
                 console.log(`${constants.GREEN_COLOR}Error${constants.COLOR_RESET} at getting subreddit \n${error}`)
             }
         })
 }
  ...
  */
 function submitBotSelfpost(potentialSubreddits) {
     ...
     if (potentialSubreddits.length >= 3) {
         ...
         title += "Top Subs: " + first + ", " + second + " and " + third + " upvotes"
     }
     else {
         title += "Wrong outcome, Please report a bug on github"
     }
 
     footer = "\n___\n^^&nbsp;[programmer](https://www.reddit.com/message/compose/?to" + constants.PROGRAMMER + "=)&nbsp;|&nbsp;[" + constants.R_BOT_SUBREDDIT + "](https://www.reddit.com/" + constants.R_BOT_SUBREDDIT + ")&nbsp;"
     if (constants.GITHUB_LINK) {
         footer += "|&nbsp;[source&nbsp;code](" + constants.GITHUB_LINK + ")&nbsp;"
     }
     ...
     rClient.getSubreddit(constants.POST_TO_SUBREDDIT).submitSelfpost({ title: title, text: text })
         ...
         .then(submissionObj => {
             ...
             // replay to the earliest in the top 3 subs
             latest_comment_in_top_3 = potentialSubreddits[0]
             top_3 = potentialSubreddits.splice(0, 3)
             pos = 0
             for (let i = 0; i < top_3.length; i++) {
                 if (top_3[i].created_utc > latest_comment_in_top_3.created_utc) {
                     latest_comment_in_top_3 = top_3[i]
                     pos = i
                 }
             }
             replayToComment(latest_comment_in_top_3, pos, submissionObj.name)
         })
         .catch((error) => {
 ...
 function replayToComment(PSsubredditObj, pos, submissionID) {
     if (!bannedOnSubs.includes(PSsubredditObj.parentsubreddit.slice(2))) {      // slice the first 2 char to get rid of r/
         rClient.getSubmission(submissionID).fetch().then(submissionObj => {
 
             ranks = ["1st", "2nd", "3rd"]
 
             let postUrl = submissionObj.url
             let info = `[](#bot)r/${PSsubredditObj.subredditName} doesn't exist.\n\nYou might be interested in creating r/${PSsubredditObj.subredditName} because it ranks ${ranks[pos]} in [today's post](${postUrl}) on [${constants.R_BOT_SUBREDDIT}](https://www.reddit.com/${constants.R_BOT_SUBREDDIT}).`
